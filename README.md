...
kubectl create secret generic postgres-certs --namespace ${ENVIRONMENT_NAME} --dry-run=client --type=kubernetes.io/tls --from-file=tls.crt=postgres-${ENVIRONMENT_NAME}-client-cert.pem --from-file=tls.key=postgres-${ENVIRONMENT_NAME}-client-key.pem --from-file=ca.crt=postgres-${ENVIRONMENT_NAME}-ca.pem -o yaml | kubeseal --controller-name=sealed-secrets --controller-namespace=sealed-secrets --format yaml > sealed-secret-postgres-certs.yaml
